...
import client.BuffStatValueHolder;
...
public class ChannelCharacter extends AbstractAnimatedGameMapObject implements GameCharacter, Serializable {
    ...
    public static ChannelCharacter reconstructCharacter(final CharacterTransfer ct, final ChannelClient client) {
        ...
        ret.questInfo = (Map<Integer, String>) ct.QuestInfoEntries;
        ret.savedLocations = (int[]) ct.SavedLocations;
        ret.wishlist = (int[]) ct.Wishlist;
        ret.teleportRocks = (int[]) ct.TeleportRocks;
    ...
    public static ChannelCharacter loadFromDb(int characterId, ChannelClient client) {
        ...
        try {
            ...
            while (rs.next()) {
                final QuestStatus status = new QuestStatus(rs);
            ...
            while (rs.next()) {
                if (rs.getInt("id") != characterId) { // Not this character
                    ...
                    ret.skills.put(SkillFactory.getSkill(Skills.getBlessOfFairyForJob(ret.jobId)), new SkillEntry(maxlevel, (byte) 0));
                    break;
                }
            }
            ps.close();
        ...
        try {
            ...
            try {
                ...
                final String completelyFailMessage =
                        ChannelClient.getLogMessage(this, "[charsave] Error Rolling Back");
            ...
            try {
                ...
                System.err.println(ChannelClient.getLogMessage(this, "[charsave] Error going back to autocommit mode") +
                        e);
            }
        }
    }
    ...
    public final void updateQuest(final int questId) {
        ...
        switch (status.getState()) {
            case 0:
                ...
                break;
            case 1:
                client.write(MaplePacketCreator.startQuest(this, questId, status.getCustomData()));
                client.write(MaplePacketCreator.updateQuestInfo(this, questId, status.getNpc(), (byte) 8));
                break;
            case 2:
        ...
        dragonBloodSchedule = TimerManager.getInstance().register(new Runnable() {
            ...
            public void run() {
                if (stats.getHp() - bloodEffect.getX() > 1) {
                    ...
                    map.broadcastMessage(ChannelCharacter.this, MaplePacketCreator.showBuffeffect(getId(), bloodEffect.getSourceId(), 5), false);
                }
            }
        }, 4000, 4000);
    }
...
//	    }
    ...
    private void enforceMaxHpMp() {
        ...
        if (statups.size() > 0) {
            client.write(MaplePacketCreator.updatePlayerStats(statups, getJobId()));
        }
    }
    ...
    public void changeMapBanish(final int mapid, final String portal, final String msg) {
        sendNotice(5, msg);
        final GameMap newMap = client.getChannelServer().getMapFactory(worldId).getMap(mapid);
    ...
    public void resetStats(final int str, final int dex, final int int_, final int luk) {
        ...
        client.write(MaplePacketCreator.updatePlayerStats(newStats, false, chr.getJobId()));
    }
        ...
        if (party != null) {
            ...
            for (PartyMember partychar : party.getMembers()) {
                if (partychar.getMapid() == getMapId() && partychar.getChannel() ==
                        channel) {
                    final ChannelCharacter other = ChannelManager.getInstance(channel).getPlayerStorage().getCharacterByName(partychar.getName());
                    if (other != null) {
    ...
    public void receivePartyMemberHP() {
        ...
        for (PartyMember partychar : party.getMembers()) {
            if (partychar.getMapid() == getMapId() && partychar.getChannel() ==
                    channel) {
                ChannelCharacter other = ChannelManager.getInstance(channel).getPlayerStorage().getCharacterByName(partychar.getName());
                if (other != null) {
                    client.write(MaplePacketCreator.updatePartyMemberHP(other.getId(), other.getStats().getHp(), other.getStats().getCurrentMaxHp()));
                }
            }
        }
    }
    ...
    public final void expirationTask() {
        ...
        final List<IItem> toberemove = new ArrayList<>(); // This is here to prevent deadlock.
        ...
        for (Map.Entry<Integer, QuestStatus> entry : quests.entrySet()) {
            ...
            if (status.mobKilled(id)) {
                ...
                if (info.canComplete(this, null)) {
                    client.write(MaplePacketCreator.getShowQuestCompletion(questId));
                }
            }
        }
    }
    ...
    public void levelUp() {
        ...
        statup.add(new StatValue(Stat.LEVEL, (int) level));
        ...
        if (bHealingLvl > 0) {
            ...
            beholderHealingSchedule = TimerManager.getInstance().register(new Runnable() {
                ...
                public void run() {
                    ...
                    map.broadcastMessage(ChannelCharacter.this, MaplePacketCreator.summonSkill(getId(), 1321007, 5), true);
                    map.broadcastMessage(ChannelCharacter.this, MaplePacketCreator.showBuffeffect(getId(), 1321007, 2), false);
                }
        ...
        if (bBuffLvl > 0) {
            ...
            beholderBuffSchedule = TimerManager.getInstance().register(new Runnable() {
                ...
                public void run() {
                    ...
                    client.write(MaplePacketCreator.showOwnBuffEffect(1321007, 2));
                    map.broadcastMessage(ChannelCharacter.this, MaplePacketCreator.summonSkill(getId(), 1321007, (int) (Math.random() *
                            3) + 6), true);
                    map.broadcastMessage(ChannelCharacter.this, MaplePacketCreator.showBuffeffect(getId(), 1321007, 2), false);
                }
