...
//	}
    ...
    public final void relocHeal() {
        ...
        if (GameConstants.isJobFamily(200, playerjob)) { // Improving MP recovery
            shouldHealMP += ((float) ((float) chra.getCurrentSkillLevel(SkillFactory.getSkill(2000000)) / 10) * chra.getLevel());
    ...
    public final void connectData(final PacketBuilder builder) {
        builder.writeAsShort(str); // str
        builder.writeAsShort(dex); // dex
        builder.writeAsShort(int_); // int
        builder.writeAsShort(luk); // luk
        builder.writeAsShort(hp); // hp
        builder.writeAsShort(maxhp); // maxhp
        builder.writeAsShort(mp); // mp
        builder.writeAsShort(maxmp); // maxmp
    }
}