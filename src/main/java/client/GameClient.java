...
public class GameClient implements Serializable {
    ...
    public final List<GameCharacter> loadCharacters(final int serverId) { // TODO make this less costly zZz
        final List<GameCharacter> chars = new LinkedList<GameCharacter>();

        for (final CharNameAndId cni : loadCharactersInternal(serverId)) {
            final GameCharacter chr = GameCharacter.loadFromDb(cni.id, this, false);
            chars.add(chr);
    ...
    public int finishLogin() {
        synchronized (GameClient.class) {
            ...
            if (state > GameClient.LOGIN_NOTLOGGEDIN && state !=
                    GameClient.LOGIN_WAITING) { // already loggedin
                loggedIn = false;
                return 7;
            }
            updateLoginState(GameClient.LOGIN_LOGGEDIN, null);
        }
        return 0;
    }
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (banned > 0) {
                    ...
                    if (loginstate > GameClient.LOGIN_NOTLOGGEDIN) { // already loggedin
                        loggedIn = false;
                        loginok = 7;
                    } else {
    ...
    public final void updateLoginState(final int newstate, final String SessionID) { // TODO hide?
        ...
        if (newstate == GameClient.LOGIN_NOTLOGGEDIN || newstate ==
                GameClient.LOGIN_WAITING) {
            loggedIn = false;
            serverTransition = false;
        } else {
            serverTransition = (newstate == GameClient.LOGIN_SERVER_TRANSITION ||
                    newstate == GameClient.CHANGE_CHANNEL);
    ...
    public final byte getLoginState() { // TODO hide?
        ...
        try {
            ...
            if (state == GameClient.LOGIN_SERVER_TRANSITION || state ==
                    GameClient.CHANGE_CHANNEL) {
                if (rs.getTimestamp("lastlogin").getTime() + 20000 <
                        System.currentTimeMillis()) { // connecting to chanserver timeout
                    state = GameClient.LOGIN_NOTLOGGEDIN;
                    updateLoginState(state, null);
                }
            }
            rs.close();
            ps.close();
            if (state == GameClient.LOGIN_LOGGEDIN) {
                loggedIn = true;
            } else {
                loggedIn = false;
            }
