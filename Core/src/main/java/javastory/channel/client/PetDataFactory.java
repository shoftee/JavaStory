...
public final class PetDataFactory {
	...
	private static WzDataProvider dataRoot =
			WzDataProviderFactory.getDataProvider("Item.wz");
	...
	public static PetCommand getPetCommand(final int petId, final int skillId) {
		...
		int inc = 0;
		if (skillData != null) {
			prob = WzDataTool.getInt("interact/" + skillId + "/prob",
										skillData, 0);
			inc = WzDataTool.getInt("interact/" + skillId + "/inc", skillData,
									0);
		}
	...
	public static int getHunger(final int petId) {
		...
		final WzData hungerData = dataRoot.getData("Pet/" + petId + ".img")
