                    ...
                    if (attack.skill == 3221007) {
                        ...
                        if (totDamageToOneMonster >= 199999) {
                            ...
                            totDamageToOneMonster = Math.min(ChannelCharacter.damageCap, Math.max(totDamageToOneMonster, totDamageToOneMonster *
                                    ...
                                    (player.haveItem(ItemInfoProvider.UNLIMITED_SLOT_ITEM, 1, true, true) ? 2 : 1)));
                            if (player.isOnDMG()) {
                                player.sendNotice(5, "Damage: " +
...
//				    System.out.println("EXCEED!!! Client damage : " + eachd + " Server : " + MaxDamagePerHit);
    ...
    private static double CalculateMaxMagicDamagePerHit(final ChannelCharacter chr, final ISkill skill, final Monster monster, final MonsterStats mobstats, final ActivePlayerStats stats, final Element elem, final Integer sharpEye, final double maxDamagePerMonster) {
        ...
        if (sharpEye != null) {
            elemMaxDamagePerMob += (elemMaxDamagePerMob / 100) *
                    sharpEye;
        }
                ...
                TimerManager.getInstance().schedule(new Runnable() {
                    ...
                    public void run() {
                        player.getMap().spawnMesoDrop(Math.min((int) Math.max(((double) eachd /
                                (double) 20000) * maxmeso, 1), maxmeso), new Point((int) (mob.getPosition().getX() +
