...
public class ItemMakerHandler {
    ...
    private static boolean hasSkill(final ChannelClient c, final int reqlvl) {
        ...
        if (Jobs.isCygnus(player.getJobId())) { // KoC Maker skill.
            return player.getCurrentSkillLevel(SkillFactory.getSkill(10001007)) >=
                    reqlvl;
        } else if (Jobs.isAran(player.getJobId())) { // KoC Maker skill.
            return player.getCurrentSkillLevel(SkillFactory.getSkill(20001007)) >=
                    reqlvl;
        } else {
            return player.getCurrentSkillLevel(SkillFactory.getSkill(1007)) >=
                    reqlvl;
        }
    }
}