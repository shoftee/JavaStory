...
public final class MobAttackInfoFactory {
    ...
    private static Map<MobAttackId, MobAttackInfo> mobAttacks = new HashMap<>();
    ...
    public MobAttackInfo getMobAttackInfo(Monster mob, int attack) {
        MobAttackInfo ret = mobAttacks.get(new MobAttackId(mob.getId(), attack));
        if (ret != null) {
            return ret;
        }

        WzData mobData = dataSource.getData(StringUtil.getLeftPaddedStr(Integer.toString(mob.getId()) +
                ".img", '0', 11));
        if (mobData != null) {
            WzData infoData = mobData.getChildByPath("info/link");
            if (infoData != null) {
                String linkedmob = WzDataTool.getString("info/link", mobData);
                mobData = dataSource.getData(StringUtil.getLeftPaddedStr(linkedmob +
                        ".img", '0', 11));
            }
        ...
        mobAttacks.put(new MobAttackId(mob.getId(), attack), ret);
